(function(window) {
	...
	function AutoExpandFrequencyCapping() {
		...
		function _autoExpand() {
			...
			if (_settings.hasTimeUntilAutoCollapse()) {
				_autoCollapseTimeout = setTimeout(function() { _autoCollapse(); }, _settings.getTimeUntilAutoCollapse());
			}
			
			_dispatchEvent(_referenceToThis.Events.AUTO_EXPAND, _referenceToThis.EventTiming.AFTER);
